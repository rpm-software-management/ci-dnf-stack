...
Scenario: Group Install Remove in installroot dockertesting4
  Given I use the repository "upgrade_1"
  When I execute "dnf" command "config-manager --installroot=/dockertesting4 --add-repo /var/www/html/repo/test-1" with "success"
  When I execute "bash" command "rpm -q --root=/dockertesting4 TestC" with "fail"
  When I execute "dnf" command "install --installroot=/dockertesting4 --releasever=23 -y TestC" with "success"
  When I execute "bash" command "rpm -q --root=/dockertesting4 TestC" with "success"
  ...
  When I execute "dnf" command "--installroot=/dockertesting4 group list Testgroup" with "success"
  Then line from "stdout" should "not start" with "Installed groups:"
  And line from "stdout" should "start" with "Available groups:"
  When I execute "dnf" command "--installroot=/dockertesting4 install -y @Testgroup" with "success"
  Then I execute "bash" command "rpm -q --root=/dockertesting4 TestA TestB TestC" with "success"
  And line from "stdout" should "start" with "TestA"
  And line from "stdout" should "start" with "TestB"
  And line from "stdout" should "start" with "TestC"
  When I execute "dnf" command "group list Testgroup" with "success"
  Then line from "stdout" should "not start" with "Installed groups:"
  And line from "stdout" should "start" with "Available groups:"
  When I execute "dnf" command "--installroot=/dockertesting4 group list Testgroup" with "success"
