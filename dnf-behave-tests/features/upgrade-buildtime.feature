...
Scenario Outline: Upgrade does not reinstall package with the same NEVRA and different build time
   ...
   When I execute bash with args "SOURCE_DATE_EPOCH=$(($(rpm -q --queryformat "%{{BUILDTIME}}" {context.dnf.repos_location}/dnf-ci-fedora/noarch/setup-2.12.1-1.fc29.noarch.rpm) + <offset>)) rpmbuild -rb --define "_rpmdir ." {context.dnf.repos_location}/dnf-ci-fedora/src/setup-2.12.1-1.fc29.src.rpm" in directory "{context.dnf.tempdir}"
   Then the exit code is 0
   # make sure that build times of local and repository packages are really different
   When I execute bash with args "[[ $(rpm -q --queryformat "%{{BUILDTIME}}" {context.dnf.repos_location}/dnf-ci-fedora/noarch/setup-2.12.1-1.fc29.noarch.rpm) -ne $(rpm -q --queryformat "%{{BUILDTIME}}" noarch/setup-2.12.1-1.fc29.noarch.rpm) ]]" in directory "{context.dnf.tempdir}"
   Then the exit code is 0
