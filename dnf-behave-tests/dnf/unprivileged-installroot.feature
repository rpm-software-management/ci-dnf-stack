...
Scenario: Fail when installing into installroot as unprivileged user
  Given I use repository "miscellaneous"
   When I execute dnf with args "--releasever=32 --nogpgcheck --installroot=/home/testuser/f32 install dummy" as an unprivileged user
   Then the exit code is 1
    And stderr contains "Error: This command has to be run with superuser privileges \(under the root user on most systems\)."
