...
Scenario: Generate new multiarch manifest file
  Given I use repository "security-upgrade-multilib"
    And I successfully execute dnf with args "manifest new B-1-1 --arches=x86_64,i686"
...
Scenario: Generate new manifest files for each arch
  Given I use repository "security-upgrade-multilib"
    And I successfully execute dnf with args "manifest new B-1-1 --arches=x86_64,i686 --per-arch"
...
Scenario: Download multiarch packages from multiarch manifest
  Given I use repository "manifest-multiarch"
    And I successfully execute dnf with args "manifest new foo --arches=x86_64,ppc64"
   When I execute dnf with args "manifest download --arches=x86_64,ppc64"
   Then the exit code is 0
    And file sha256 checksums are following
        | Path                                                                        | sha256                                                                                          |
...
Scenario: Download multiarch packages from per-arch manifests
  Given I use repository "manifest-multiarch"
    And I successfully execute dnf with args "manifest new foo --arches=x86_64,ppc64 --per-arch"
    And I successfully execute dnf with args "manifest download --arches=x86_64"
    And I successfully execute dnf with args "manifest download --arches=ppc64"
