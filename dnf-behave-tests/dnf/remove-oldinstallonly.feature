...
@no_installroot
@destructive
Scenario: Running kernel is skipped and warning is emitted
  Given I use repository "dnf-ci-fedora"
    And I fake kernel release to "4.19.15-300.fc29.x86_64"
    And I set config option "installonly_limit" to "3"
  ...
  Given I use repository "dnf-ci-fedora-updates-testing"
   ...
   Then the exit code is 0
    And Transaction is following
        ...
        | unchanged     | kernel-core-0:4.19.15-300.fc29.x86_64 |
        | unchanged     | kernel-core-0:4.18.16-300.fc29.x86_64 |
   When I execute dnf with args "remove --oldinstallonly --limit=1"
   Then the exit code is 0
    And Transaction is following
        | Action        | Package                               |
        | unchanged     | kernel-core-0:4.20.6-300.fc29.x86_64  |
        | unchanged     | kernel-core-0:4.19.15-300.fc29.x86_64 |
        | remove        | kernel-core-0:4.18.16-300.fc29.x86_64 |
    And stderr contains "Warning: The currently running kernel '4.19.15-300.fc29.x86_64' is not the latest installed version and was not removed."
    And stderr contains "Please reboot into the latest kernel and run this command again to complete the cleanup."
